...
namespace WJ_DSRCProtocolTest_Desk_Net
{
    ...
    public partial class WanjiCpcStandardTestForm : Form
    {
        ...
        /// <summary>显示选中 CPC 卡用例的移植说明入口。</summary>
        ...
        private async Task RunSelectedAsync(bool allChecked)
        {
            var nodes = new List<TreeNode>(); Collect(treeViewTestItems.Nodes, nodes, allChecked);
            if (nodes.Count == 0 && treeViewTestItems.SelectedNode != null && treeViewTestItems.SelectedNode.Nodes.Count == 0) nodes.Add(treeViewTestItems.SelectedNode);
            if (nodes.Count == 0) { richTextBoxInformation.AppendText("\r\n请先选择 CPC 卡测试叶子。\r\n"); return; }
            testCancellation = new CancellationTokenSource(); buttonSingleTest.Enabled = buttonAllTests.Enabled = false; buttonStopTest.Enabled = true;
            try { foreach (TreeNode n in nodes) { testCancellation.Token.ThrowIfCancellationRequested(); richTextBoxInformation.AppendText("\r\n开始：" + n.FullPath + "（框架占位，未调用设备）"); await Task.Delay(50, testCancellation.Token); richTextBoxInformation.AppendText(" -> 待移植"); } }
            catch (OperationCanceledException) { richTextBoxInformation.AppendText("\r\n已停止。\r\n"); }
