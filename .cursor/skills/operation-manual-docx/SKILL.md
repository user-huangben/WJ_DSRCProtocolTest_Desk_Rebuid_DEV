...
根据当前工程已实现界面和测试流程，在 `.sln` 同级生成 `操作说明.docx`（当前为 `WJ_DSRCProtocolTest_Desk_Net/操作说明.docx`）。使用 `scripts/build_manual.py` 生成文档，截图及标注后的生成物放在 `WJ_DSRCProtocolTest_Desk_Net/artifacts/manual/`。只写代码能验证的功能，不把占位用例写成已实现。
...
`python scripts/render_docx.py WJ_DSRCProtocolTest_Desk_Net/操作说明.docx --output_dir WJ_DSRCProtocolTest_Desk_Net/artifacts/manual/rendered`
